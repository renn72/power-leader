...
import { getAge, getTotalWilks } from '~/lib/utils'
...
const LeaderBoardResults = ({
    ...
    .filter((entry) => {
      let liftType = ''
			...
			if (isDeadlift) liftType = 'deadlift'
			if (isBench) liftType = 'bench'
			if (isBench && isDeadlift) liftType = 'pushPull'
			if (isSquat && isBench && isDeadlift) liftType = '3lift'

      console.log(competition.compDayInfo.resultsLift, liftType)
      return competition.compDayInfo.resultsLift === '' ? true : competition.compDayInfo.resultsLift === liftType
    })
		.filter((entry) => {
			if (age === '') return true
  ...
  const liftType = competition.compDayInfo.resultsLift
		...
		<>
			<div className='h-[calc(100vh-40px)] relative w-full'>
				...
				<div className='flex flex-col gap-6 items-center justify-center h-[calc(100vh-40px)] relative px-4 lg:px-0 w-full'>
					<div className='text-5xl font-bold capitalize mb-24 rounded-full py-4 px-16 bg-yellow-500 text-black'>
						{liftType === '' ? null : (
							<div className='text-center mb-4'>
								{liftType === '3lift' ? '3 Lift' :
                liftType === 'pushPull' ? 'Push Pull' :
                liftType === 'squat' ? 'Squat Only' :
                liftType === 'bench' ? 'Bench Only' :
                liftType === 'deadlift' ? 'Deadlift Only' : ''}

							</div>
						)}
						<div className='flex gap-4'>
							<div>{table === 'all' ? '' : table}</div>
							<div>{gender}</div>
							<div>{wc && wc !== '' ? wc === 'SHW' ? `${wc}` : `${wc}kg` : ''}</div>
							<div>{age}</div>
						</div>
					</div>
					{entries.map((entry, index) => (
						<LeaderBoardRow
							...
							index={index}
							key={entry.id}
							reveal={competition.compDayInfo.resultsReveal || 'hidden'}
						/>
					))}
				</div>
			</div>
