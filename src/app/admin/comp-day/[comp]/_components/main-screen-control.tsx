...
const MainScreenControl = ({
	...
	useEffect(() => {
		...
		if (isJudged && !isTicking && isAuto) {
			if (Number(index) + 1 < entries.length) {
				setIsTicking(true)
				setTimeout(() => {
					console.log('tick')
					setIsTicking(false)
				...
				if (competition.compDayInfo.round < 3) {
					setIsTicking(true)
					setTimeout(() => {
						console.log('tick')
						setIsTicking(false)
						updateLift({
							id: competition.id,
							uuid: competition.uuid || '',
							round: competition.compDayInfo.round + 1,
							lift: competition.compDayInfo.lift,
							bracket: competition.compDayInfo.bracket,
							index: 0,
							nextIndex: 1,
						})
            setRound((competition.compDayInfo.round + 1).toString())
					}, 6000)
				}
			}
		} else {
	...
	console.log('info', competition.compDayInfo)
	...
	return (
		<div className='flex flex-row items-center gap-2 col-span-3'>
			<div className='flex items-center justify-around gap-2 rounded-md border border-input p-2'>
				...
				>
					<ToggleGroupItem className='h-8 xl:h-12' value='squat'>
						Squat
					</ToggleGroupItem>

					<ToggleGroupItem className='h-8 xl:h-12' value='bench'>
						Bench
					</ToggleGroupItem>
					<ToggleGroupItem className='h-8 xl:h-12' value='deadlift'>
						Deadlift
					</ToggleGroupItem>
				</ToggleGroup>
			</div>
          ...
          value={round}
				...
				>
					<ToggleGroupItem className='h-8 xl:h-12' value='1'>
						1
					</ToggleGroupItem>
					<ToggleGroupItem className='h-8 xl:h-12' value='2'>
						2
					</ToggleGroupItem>
					<ToggleGroupItem className='h-8 xl:h-12' value='3'>
						3
					</ToggleGroupItem>
					<ToggleGroupItem className='h-8 xl:h-12' value='4'>
						4
					</ToggleGroupItem>
				</ToggleGroup>
			</div>
