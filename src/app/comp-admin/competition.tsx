              ...
              <div className='grid grid-cols-4 gap-2'>
                {competition.wc_male?.split('/').map((event) => (
                  <div
                    ...
                    className='text-nowrap'
                  >
                    <Badge className='w-14 justify-center'>{event}kg</Badge>
                  </div>
                ))}
              </div>
            </HoverCardContent>
          ...
          <HoverCard>
            <HoverCardTrigger>
              <div className='flex gap-1'>
                ...
                {competition.wc_female &&
                  competition.wc_female?.split('/')?.length > 3 && (
                    <div className='text-nowrap'>...</div>
                  )}
              </div>
            </HoverCardTrigger>
            <HoverCardContent className='w-80'>
              <div className='grid grid-cols-4 gap-2'>
                {competition.wc_female?.split('/').map((event) => (
                  <div
                    key={event}
                    className='text-nowrap'
                  >
          ...
          <HoverCard>
            <HoverCardTrigger>
              <div className='flex flex-col gap-1'>
                {competition.divisions?.slice(0, 3).map((division) => (
                  <div
                    key={division.id}
                    className='text-nowrap'
                  >
            ...
            <HoverCardContent>
              <div className='flex flex-col gap-1'>
                ...
                {competition.divisions.map((division, i) => (
                  ...
                  >
                    {i !== 0 && <Separator className='col-span-6' />}
                    <div className='col-span-2'>{division.name}</div>
                    <div>{division.minAge || '-'}</div>
                    <div>{division.maxAge || '-'}</div>
                    <div className='col-span-2'>{division.info}</div>
                  </div>
                ))}
              </div>
            </HoverCardContent>
        ...
        <TableCell className='hidden'>
          {(competition.currentState === 'created' ||
            ...
            <div className='flex flex-col items-center gap-2'>
              <div className='font-bold text-destructive'>Closed</div>
              <Button
